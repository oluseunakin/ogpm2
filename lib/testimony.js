...
export async function testimonyByName(name) {
  return await by('testimonies', name.toUpperCase(), 'testifier')
}

export const shareTestimony = async ({ testifier, testimony, media,desc,date }) => {
  const minGallery = [];
  try {
    const testimonyRef = await getRef("testimonies").add({ testifier: testifier.toUpperCase(), desc, testimony, date, minGallery})
    if(media) {
